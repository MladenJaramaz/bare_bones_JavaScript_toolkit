...
// generate a detached copy of an object, so that changes to the copy do not affect the original object
...
// remove properties from restrictedObj, if there is not a property with that name in restrictingObj
function objRestrict(restrictedObj, restrictingObj) {

    for (let prop in restrictedObj) {
        if (prop in restrictingObj) continue;
        delete restrictedObj[prop];
    }
    return restrictedObj;
}
// remove properties from subtractionAffectedObj, if there is a property with that name in subtractionDefiningObj
function objSubtract(subtractionAffectedObj, subtractionDefiningObj) {

    for (let prop in subtractionAffectedObj) {
        if (prop in subtractionDefiningObj) delete subtractionAffectedObj[prop];
    }
    return subtractionAffectedObj;
}
...
// create a private property for an object using closures, create property accessor methods for it
...
function addPrivateProperty(obj, propertyName, setterCheckFunction) {
    ...
    let privateProperty;
    obj[`get${propertyName}`] = function() { return privateProperty; };
    obj[`set${propertyName}`] = function(newValue) {
        
        if (typeof setterCheckFunction !== 'function') throw new TypeError('No valid setter checking function provided.');
        if (!setterCheckFunction(newValue)) throw new Error(`set${propertyName} recognized value ${newValue} as an invalid value`);
