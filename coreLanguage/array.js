'use strict';
// POSSIBLE IMPROVMENTS: add check if arguments are true arrays for all of the functions below
...
function arrMax(arr) {return arr.reduce((x, y) => {return (x > y ? x : y);})}
function arrMax2(arr) {return Math.max.apply(Math, arr);}
...
function arrStandardDeviation(arr) {
    ...
    const squaredDeviations = arr.map((value) => {return Math.pow((value - mean), 2);}); // irrelevant if devation is positive or negative value, because it will be squared in the next steps
...
// find every occurence of a value in an array amd return an array of matching indexes
